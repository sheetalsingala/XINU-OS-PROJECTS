...
#define PLWAIT      '\011'		/* process is waiting on a lock */
...
struct	pentry	{
	...
	int	pwaitret;
	int plock; 	/*Lock descriptor of lock that the process is currently waiting on */
	int pltype; /*Type of lock waiting on*/
	int plwaitret;		/* return for lock */
	int lacquired[50];  /* List of all locks arrived */
	int pinh; 	/* Dynamic priority, if assigned */
	int plwaittime; /*Wait time for a lock */
};
