...
//int ptcount  = 0;	/* Number of frames */
...
#endif
	/* Initialize bsm_tab and frm_tab */
	init_bsm();
	init_frm();
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pptr->pargs = 0;
	pptr->pprio = 0;
	/* Creation of null process page directory and pointing pbdr to it  */
	int nullpd = initPD(0);
	pptr->pdbr = (nullpd + 1024) * 4096;
	//kprintf("\nNull PD%08x", pptr->pdbr);
	pptr->vhpno = 0;
	pptr->vhpnpages = 0;
	pptr->store = -1;
	write_cr3(pptr->pdbr);		/* Store null process pd to cr3 */
        //unsigned long cr3written = read_cr3();
        //kprintf("\nCR3 value = %08x", cr3written);
	currpid = NULLPROC;
	...
	/* Creation ogf global page table */
	initglobaltable();
