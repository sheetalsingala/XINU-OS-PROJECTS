...
//  }
 ...
 // kprintf("\nstore %d page %d vaddr %08x", store, pageth, vaddr);
  if(store == -1){
  ...
  get_frm(&frameent);   /* Either the page is not present or PT is not present, so get a free frame */
      ...
      frm_tab[frameent].fr_status = FRM_MAPPED;
	    ...
	    ptent->pt_user = 0;
            ...
            ptent->pt_global = 0;
            ptent->pt_avail = 3;
  	    //kprintf("\nptent %d frame val %d", ptent, (FRAME0+7)*NBPG);     
	    ...
	    //kprintf("\nFrame count %d frame number %d", frm_tab[pdent->pd_base - FRAME0].fr_refcnt, pdent->pd_base - FRAME0);      
  //          frm_tab[frameent].fr_refcnt = 0; 
